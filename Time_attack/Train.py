...
attack_time_data = sio.loadmat("time_Mix_0.5.mat")["data"]
...
# # 高斯噪声的对抗样本产生
# gauss_nn = np.zeros([test_Images.shape[0], 5, 22, 256])
# for i in range(test_Images.shape[0]):
#     for r in range(test_Images.shape[1]):
#         for e in range(5):
#             gauss_nn[i][e][r] = test_Images[i][r][0 + 256 * e:256 + 256 * e] + random.gauss(mu=0, sigma=p)
# gauss_nn = gauss_nn[:, :, np.newaxis, :, :]  #

# # BEAM未转换时的失真度
# o = 0
# SUM = 0
# for i in range(test_Images.shape[0]):
#     for n in range(test_Images.shape[1]):
#         for k in range(test_Images.shape[2]):
#             RMSE = mean_squared_error(test_Images[i][n][k].flatten(), attack_time_data[i][n][k].flatten()) ** 0.5
#             SUM = SUM + RMSE
#             o = o + 1
# print(SUM/o)
#
...
# o = 0
# SUM = 0
# for i in range(gauss_nn.shape[0]):
#     for n in range(gauss_nn.shape[1]):
#             RMSE = mean_squared_error(gauss_nn[i][n].flatten(), test_nn[i][n].flatten()) ** 0.5
#             SUM = SUM + RMSE
#             o = o + 1
# print(SUM/o)
...
# Gauss = EEGImagesDataset(test_Label, gauss_nn)
...
test_loader = torch.utils.data.DataLoader(Test, batch_size=16,
                                          shuffle=False, num_workers=0)
# gauss_loader = torch.utils.data.DataLoader(Gauss, batch_size=16,
#                                            shuffle=False, num_workers=0)
