...
class MaxCNN(nn.Module):
    ...
    return x：对数softmax后最后一层的输出。
    """
    def __init__(self, input_image=torch.zeros(1, 5, 4, 22, 22), kernel=(3,3), stride=1, padding=1,max_kernel=(2,2), n_classes=2):
        ...
        self.fc = nn.Linear(n_window*int(2*2*128/n_window),128)
        self.fc2 = nn.Linear(128,n_classes)
        self.max = nn.LogSoftmax()

    def forward(self, x):
        if x.get_device() == 0:
            tmp = torch.zeros(x.shape[0],x.shape[1],128,2,2).cuda()
        else:
            tmp = torch.zeros(x.shape[0],x.shape[1],128,2,2).cpu()
        for i in range(5):
        ...
        x = tmp.reshape(x.shape[0], x.shape[1],2*128*2,1)
        x = self.pool(x)
...
class TempCNN(nn.Module):
    ...
    def __init__(self, input_image=torch.zeros(1, 5, 4, 22, 22), kernel=(3,3), stride=1, padding=1,max_kernel=(2,2), n_classes=2):
        ...
        #Temporal CNN Layer
        self.conv8 = nn.Conv1d(n_window,64,(2*2*128,3),stride=stride,padding=padding)
...
class LSTM(nn.Module):
    ...
    return x：对数softmax后最后一层的输出。
    """
    def __init__(self, input_image=torch.zeros(1, 5, 4, 22, 22), kernel=(3,3), stride=1, padding=1,max_kernel=(2,2), n_classes=2, n_units=128):
        ...
        # LSTM Layer
        self.rnn = nn.RNN(2*2*128, n_units, n_window)
    ...
    def forward(self, x):
        ...
        for i in range(5):
            ...
            del img
        x = tmp.reshape(x.shape[0], x.shape[1], 2 * 128 * 2)
        del tmp
...
class Mix(nn.Module):
    ...
    def __init__(self, input_image=torch.zeros(1, 5, 4, 22, 22), kernel=(3,3), stride=1, padding=1,max_kernel=(2,2), n_classes=2, n_units=128):
        ...
        # Temporal CNN Layer
        self.conv8 = nn.Conv2d(n_window, 64, (2 * 2 * 128, 3), stride=stride, padding=padding)
    ...
    def forward(self, x):
        ...
        temp_conv = F.relu(self.conv8(tmp.reshape(x.shape[0], x.shape[1], 2 * 128 * 2, 1)))
        temp_conv = temp_conv.reshape(temp_conv.shape[0], -1)

        self.lstm_out, _ = self.rnn(tmp.reshape(x.shape[0], x.shape[1], 2 * 128 * 2))
        del tmp
