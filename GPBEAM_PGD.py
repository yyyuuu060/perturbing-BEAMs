...
def PGD(model, X, y, test_data,
    ...
    for _ in range(num_steps):
        ...
        for u in range(test_data.shape[0]):
            ...
            # dd = np.concatenate([x_test[u][0], X_test[u][0]], axis=0) # Paint the BEAMs and BEAMs Adversarial sample
            # create_montage(dd)
...
def eval_adv_test(model, device, test_loader, test_data):
    """
    ...
    for i, data in enumerate(test_loader, 0):
        ...
        attack_images, X_test = PGD(model, X, y, test_data[0 + i * 64:64 + i * 64])
...
if __name__ == '__main__':
    ...
    eval_adv_test(test_net, device, test_loader, x_test)
